...
\subsubsection{NTT for $4$-element word arrays}
...
\[ [A[k], B[k], C[k], D[k]] \]
where $A,B,C,D$ are the $4$ input vectors.    Algorithm~\ref{} remains exactly the same, with the assumption that
all operations are now executed in a SIMD fashion.

\noindent \textbf{Single NTT for $4$-element word arrays}
